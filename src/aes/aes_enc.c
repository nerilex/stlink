...
#define PRECOMPUTED_T_TABLES 1

#if PRECOMPUTED_T_TABLES
...
#define precompute_tables()
#define AES_SBOX(a) (T1[(a)] & 0xff)
...
static void print_T_tables()
{
    uint32_t *t[4] = {T0, T1, T2, T3};
    int i, j;
    for (i = 0; i < 4; ++i) {
        printf("\nstatic const uint32_t T%d[256] = {\n", i);
        for (j = 0; j < 255; ++j) {
            printf(((j + 1) % 16 == 0) ? "0x%08x,\n" : "0x%08x, ", t[i][j]);
        }
        printf("0x%08x\n};\n", t[i][255]);
    }
}
